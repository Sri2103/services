...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if query := oiq.withProduct; query != nil {
		if err := oiq.loadProduct(ctx, query, nodes,
			func(n *OrderItem) { n.Edges.Product = []*Product{} },
			func(n *OrderItem, e *Product) { n.Edges.Product = append(n.Edges.Product, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Product)) error {
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(orderitem.ProductColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_item_product
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_item_product" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_item_product" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
