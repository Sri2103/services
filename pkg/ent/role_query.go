...
func (rq *RoleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	for _, n := range neighbors {
		fk := n.role_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
